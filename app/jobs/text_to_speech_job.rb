class TextToSpeechJob < ApplicationJob
  ...
  def perform(prompt, voice_id)
    ...
    audio_response = send_text_to_speech_request(gpt_text, voice_id)
    create_audio_file(prompt, audio_response)
  end
  ...
  def send_text_to_speech_request(gpt_text, voice_id)
    Faraday.post("https://api.elevenlabs.io/v1/text-to-speech/#{voice_id}") do |req|
  ...
  def create_audio_file(prompt, response)
